PL/pgSQL. Функции и процедуры – Telegraph
...
create sequence t1\_id\_seq;

create table t2 (
id bigserial primary key,
t1\_id bigint references t1 (id)
);
...
id1 bigint = nextval('t1\_id\_seq'); -- Инициализируем переменную
...
create procedure insert\_data()
language plpgsql
as $$
declare
id1 bigint = nextval('t1\_id\_seq');
id2 bigint = nextval('t1\_id\_seq');
begin
insert into t1 values
(id1, 1),
(id2, 1);
...
create procedure insert\_data\_with\_params(id1 bigint, id2 bigint)
language plpgsql
as $$
begin
insert into t1 values
...
create procedure insert\_data\_with\_params\_sql(id1 bigint, id2 bigint)
language sql
as $$
insert into t1 values
(id1, 1),
...
alter procedure insert\_data() rename to insert\_data\_plpgsl;
...
drop procedure insert\_data\_plpgsl;

И, наконец, вызов процедуры осуществляется через оператор **_CALL_**:

call insert\_data\_plpgsl();
call insert\_data\_with\_params(100, 101);
call insert\_data\_with\_params\_sql(200, 201);
...
create function sum\_params(a bigint, b bigint) returns bigint
language plpgsql
as $$
begin
return a + b;
...
create function next\_t1\_id\_seq() returns bigint
language plpgsql
as $$
begin
return nextval('t1\_id\_seq');
end
$$;
...
create function insert\_data\_f() returns boolean
language plpgsql
as $$
declare
id1 bigint = next\_t1\_id\_seq();
id2 bigint = next\_t1\_id\_seq();
begin
insert into t1 values
(id1, 1),
(id2, 1);
...
create or replace function insert\_data\_f() returns boolean
language plpgsql
as $$
begin
call insert\_data\_plpgsl();
...
alter function insert\_data\_f() rename to insert\_data;

То же и с удалением:

drop function insert\_data;
...
select sum\_params(1, 2);



С теорией на сегодня все!

![](/file/26268caf09ea729ebc459.png)
...
Please submit your DMCA takedown request to [\[email protected\]](/cdn-cgi/l/email-protection#)

Cancel Report

var T={"apiUrl":"https:\\/\\/edit.telegra.ph","datetime":1696095679,"pageId":"4fce8e35282d4038f4e90","editable":true};(function(){var b=document.querySelector('time');if(b&&T.datetime){var a=new Date(1E3\*T.datetime),d='January February March April May June July August September October November December'.split(' ')\[a.getMonth()\],c=a.getDate();b.innerText=d+' '+(10>c?'0':'')+c+', '+a.getFullYear()}})();