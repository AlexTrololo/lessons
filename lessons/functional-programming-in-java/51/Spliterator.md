...
2. _Iteratoriterator()_. Возвращает объект типа _Iterator_, позволяющий обрабатывать элементы, содержащимися в наследнике _Iterable_, в определенном (реализацией итератора) порядке. Подробнее мы рассмотрим интерфейс _Iterator_ в следующем пункте статьи;
